...
void StatisticsDialog::update(Range range)
{
    ...
    for (const auto& info : _parent->curveList())
    {
        ...
        if (stat.count > 0)
        {
            ...
            stat.median = abs_values[abs_values.size() / 2];  // 50%��λ��
            stat.p68 = abs_values[static_cast<int>(abs_values.size() * 0.68)];  // 68%��
            stat.p95 = abs_values[static_cast<int>(abs_values.size() * 0.95)];  // 95%��
        }
