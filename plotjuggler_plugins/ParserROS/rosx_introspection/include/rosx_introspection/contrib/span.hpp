...
// MSVC++  7.0  _MSC_VER == 1300  span_COMPILER_MSVC_VERSION ==  70  (Visual Studio .NET
// 2002) MSVC++  7.1  _MSC_VER == 1310  span_COMPILER_MSVC_VERSION ==  71  (Visual Studio
// .NET 2003) MSVC++  8.0  _MSC_VER == 1400  span_COMPILER_MSVC_VERSION ==  80  (Visual
// Studio 2005) MSVC++  9.0  _MSC_VER == 1500  span_COMPILER_MSVC_VERSION ==  90  (Visual
// Studio 2008) MSVC++ 10.0  _MSC_VER == 1600  span_COMPILER_MSVC_VERSION == 100  (Visual
// Studio 2010) MSVC++ 11.0  _MSC_VER == 1700  span_COMPILER_MSVC_VERSION == 110  (Visual
// Studio 2012) MSVC++ 12.0  _MSC_VER == 1800  span_COMPILER_MSVC_VERSION == 120  (Visual
// Studio 2013) MSVC++ 14.0  _MSC_VER == 1900  span_COMPILER_MSVC_VERSION == 140  (Visual
// Studio 2015) MSVC++ 14.1  _MSC_VER >= 1910  span_COMPILER_MSVC_VERSION == 141  (Visual
// Studio 2017) MSVC++ 14.2  _MSC_VER >= 1920  span_COMPILER_MSVC_VERSION == 142  (Visual
// Studio 2019)
...
#define span_HAVE_DEDUCTION_GUIDES                                                       \
  (span_CPP17_OR_GREATER && !span_BETWEEN(span_COMPILER_MSVC_VER, 1, 1913))
#endif
...
{
  typedef
      typename std11::remove_volatile<typename std11::remove_const<T>::type>::type type;
};
...
template <class C>
struct has_size_and_data<C, std17::void_t<decltype(std17::size(std::declval<C>())),
...
struct is_compatible_element<C, E,
...
public:
  ...
  {
    span_EXPECTS((ptr == span_nullptr && count == 0) ||
  ...
  span_constexpr span(with_container_t, Container const& cont)
    : data_(cont.size() == 0 ? span_nullptr :
...
#else
  span_constexpr span(span const& other) span_noexcept : data_(other.data_),
                                                         size_(other.size_)
  {
  }
  ...
  span_constexpr_exp span<element_type, Count> subspan() const
  {
    span_EXPECTS((detail::is_positive(Offset) && Offset <= size()) &&
                 (Count == dynamic_extent ||
                  (detail::is_positive(Count) && Count + Offset <= size())));
      ...
      size_type offset, size_type count = static_cast<size_type>(dynamic_extent)) const
  {
    ...
    return span<element_type, dynamic_extent>(
        data() + offset,
        count == static_cast<size_type>(dynamic_extent) ? size() - offset : count);
  }
...
span(It, EndOrSize) -> span<
    typename std11::remove_reference<typename std20::iter_reference_t<It> >::type>;
...
inline span_constexpr bool same(span<T1, E1> const& l,
                                span<T2, E2> const& r) span_noexcept
{
...
template <class T, extent_t Extent>
span_constexpr span<T> subspan(span<T, Extent> spn, size_t offset,
...
span_constexpr auto subspan(T& t)
...
{
  return span<const std17::byte, span_sizeof(t)>(reinterpret_cast<std17::byte const*>(&t),
                                                 span_sizeof(T));
}
...
public:
  ...
  static_assert(Extent != nonstd::dynamic_extent && I < Extent, "tuple_element<I,span>: "
                                                                "dynamic extent or index "
                                                                "out of range");
#endif
...
{
#if span_HAVE(STATIC_ASSERT)
  static_assert(Extent != nonstd::dynamic_extent && I < Extent, "get<>(span): dynamic "
                                                                "extent or index out of "
                                                                "range");
#endif
  return spn[I];
}
